...
export function RoomDesignPage() {
  ...
  return <main className="page"><h1>设计你的第一间客房</h1><p>8×12 网格，每格 0.25㎡</p><div className="toolbar"><button onClick={preset}>云岫商务房</button><label>房型名称<input aria-label="房型名称" value={name} onChange={e=>setName(e.target.value)} /></label></div><div className="grid" style={{gridTemplateColumns:`repeat(${prototypeConfig.roomColumns}, 1fr)`}}>{Array.from({length:96},(_,i)=>{const x=i%12,y=Math.floor(i/12),on=cells.some(v=>v.x===x&&v.y===y); return <button aria-label={`格子 ${x},${y}`} className={on?'cell on':'cell'} onClick={()=>toggle(x,y)} />})}</div>{metrics&&<p>面积 {metrics.areaSquareMeters}㎡ · 造价 ¥{metrics.buildCostCents/100} · 建议房价 ¥{metrics.suggestedRateCents/100} · 适配度 {metrics.businessFitBps/100}%</p>}<button onClick={()=>commands.saveRoomBlueprint(name, cells)}>保存并进入楼层</button>{error&&<p role="alert">{error}</p>}<div className="preview">房间视觉预览占位</div></main>;
}
