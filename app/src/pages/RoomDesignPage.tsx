...
import { RoomToolRail } from '../components/design/RoomToolRail';
...
export function RoomDesignPage() {
  ...
  const saveSeries = async () => {
    await commands.saveRoomSeries({ id: 'room-master-1', name: draft.name, cells: draft.cells, gene: STYLE_PRESETS.find(p => p.id === selectedPreset)?.gene ?? CONTEMPORARY_ORIENTAL.gene });
    setSavedNotice(true);
  };
  ...
  return <main className="page design-page">
    ...
    <div className="editor-layout">
      <RoomToolRail activeZone={draft.activeZone} tool={draft.tool} onZone={setActiveZone} onTool={tool => { setTool(tool); setRectangleStart(null); }} />
      <section className="canvas-panel" aria-label="房型画布" role="region"><div className="canvas-toolbar"><span>1 格 = 1㎡</span><span>{rectangleStart ? '请选择第二个角点' : '点击格子开始绘制'}</span></div><div className="grid design-grid" style={{ gridTemplateColumns: 'repeat(8,1fr)' }}>{Array.from({ length: 96 }, (_, i) => { const x = i % 8, y = Math.floor(i / 8), cell = draft.cells.find(c => c.x === x && c.y === y); return <button key={`${x}-${y}`} aria-label={`格子 ${x},${y}`} aria-pressed={Boolean(cell)} data-zone={cell?.zone} className={cell ? `cell on ${cell.zone}` : 'cell'} onClick={() => editCell(x, y)} />; })}</div><p className="canvas-note">拖拽或点击格子划分功能区；房间大小、材质与风格会共同影响造价和后续房价。</p></section>
    ...
    <div className="editor-actions"><button aria-label="撤销" disabled>撤销</button><button aria-label="重做" disabled>重做</button><button aria-label="保存客房系列" onClick={() => void saveSeries()}>保存客房系列</button><button aria-label="保存并进入楼层" onClick={() => void commands.saveRoomBlueprint(draft.name, draft.cells)}>保存并进入楼层</button></div>
