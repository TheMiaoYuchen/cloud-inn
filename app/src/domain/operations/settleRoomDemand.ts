...
import {
  GUEST_SEGMENT_IDS,
  type LostBookingReason,
  type OperationsState,
  type RoomBooking,
  type SegmentDayResult,
...
export interface RoomDemandSettlementInput {
  day: number;
  operations: Readonly<OperationsState>;
  offers: ReadonlyArray<Readonly<RoomOffer>>;
}

export interface RoomDemandSettlementResult {
  ...
  reviews: Array<{ segmentId: (typeof GUEST_SEGMENT_IDS)[number]; ratingBps: number; text: string }>;
  discoveredNeeds: OperationsState["discoveredNeeds"];
  reputationDeltaBps: number;
  renovationProgress: OperationsState["offerUpgrades"];
  segmentMix: OperationsState["segmentMix"];
  serviceBps: number;
}
...
function validateInput(input: Readonly<RoomDemandSettlementInput>): void {
  ...
  if (input.operations.maximumReputationBps > 10_000) throw new Error("最高声誉必须是 0 到 10000 的安全整数");
  ...
  if (input.day <= previousReportDay) throw new Error("营业日必须晚于最后一份经营日报");
...
function maximumAcceptedRateCents(segment: (typeof GUEST_SEGMENTS)[number]): number {
  return 240_000 - segment.priceSensitivityBps * 10;
}

export function settleRoomDemand(
  input: Readonly<RoomDemandSettlementInput>,
): RoomDemandSettlementResult {
  validateInput(input);
  ...
  const candidates = GUEST_SEGMENTS.flatMap((segment, segmentIndex) =>
    Array.from({ length: segments[segmentIndex].demand }, (_, demandIndex) =>
      pricedOffers.flatMap((offer) => {
        ...
        if (!match.eligible || offer.nightlyRateCents > maximumAcceptedRateCents(segment)) return [];
        return [{
          segmentIndex,
          demandIndex,
          offer,
  ...
  for (const candidate of candidates) {
    ...
    if (soldOffers.has(candidate.offer.id) || soldRoomsInventory.has(candidate.offer.sourceRoomId) || soldDemand.has(demandKey)) continue;
  ...
  for (const result of segments) {
    result.averageRateCents = result.soldRooms === 0 ? 0 : Math.trunc(result.revenueCents / result.soldRooms);
    const segmentIndex = GUEST_SEGMENT_IDS.indexOf(result.segmentId);
    const scores = assignments.filter((assignment) => assignment.segmentIndex === segmentIndex).map(({ score }) => score);
    result.satisfactionBps = scores.length === 0
      ? 0
      : Math.trunc((Math.trunc(scores.reduce((sum, score) => sum + score, 0) / scores.length) + service.overallBps) / 2);
  }
  ...
  const lostBookings: LostBookingReason[] = segments.flatMap((result): LostBookingReason[] => {
    ...
    const affordableOffers = eligibleOffers.filter(({ nightlyRateCents }) => nightlyRateCents <= maximumAcceptedRateCents(segment));
    if (pricedOffers.length === 0) return [{ segmentId: result.segmentId, code: "no-inventory", count: result.demand, explanation: `没有可售客房，未能承接${segment.displayName}的需求` }];
    if (eligibleOffers.length === 0) return [{ segmentId: result.segmentId, code: "hard-requirement", count: result.demand, explanation: result.segmentId === "family" ? "没有客房同时满足家庭出游的容量与床型硬要求" : `没有客房满足${segment.displayName}的入住硬要求` }];
    if (affordableOffers.length === 0) return [{ segmentId: result.segmentId, code: "price", count: result.demand, explanation: `可用客房价格超出${segment.displayName}的接受范围` }];
  ...
  const roomRevenueCents = safeNumber(
  ...
  const discoveredNeeds = reviewedSegments.flatMap((result) => {
  ...
  const renovationProgress = Object.fromEntries(
    Object.entries(input.operations.offerUpgrades).map(([key, upgrade]) => {
      if (roomOfferRenovationKind(upgrade) === null) return [key, structuredClone(upgrade)];
      return [key, { ...upgrade, remainingClosureDays: Math.max(0, (upgrade.remainingClosureDays ?? 0) - 1) }];
    }),
  );
  let segmentMix = input.operations.segmentMix === undefined
    ? undefined
    : { ...input.operations.segmentMix };
  if (soldRooms > 0) {
    let allocatedBps = 0;
    let lastSoldIndex = 0;
    ...
    segmentMix = Object.fromEntries(segments.map((segment, index) => {
  ...
  return {
    segments,
    roomRevenueCents,
    departmentCostCents: safeNumber(BigInt(service.dailyCostCents), "部门成本"),
    availableRooms,
    soldRooms,
    occupancyBps: availableRooms === 0 ? 0 : Math.trunc((soldRooms * 10_000) / availableRooms),
    lostBookings,
    reviews,
    bookings: assignments.map((assignment) => ({
