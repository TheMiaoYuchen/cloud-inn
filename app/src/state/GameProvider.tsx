...
export function GameProvider({ children, savePort: port, saveId = 'save-1', visualProvider, allowMemoryFallback = false }: { children: React.ReactNode; savePort: SavePort; saveId?: string; visualProvider?: VisualProvider; allowMemoryFallback?: boolean }) {
  ...
  const [commandPending, setCommandPending] = useState(false); const stateRef = useRef(state); stateRef.current = state; const queue = useRef(Promise.resolve()); const pendingCount = useRef(0);
  useEffect(() => { let alive = true; setLoading(true); setState(null); stateRef.current=null; setActivePort(port); initialize(port, saveId, allowMemoryFallback).then(result => { if (alive) { setState(result.state); stateRef.current=result.state; setActivePort(result.port); setError(result.warning); setLoading(false); } }, e => { if (alive) { setState(null); stateRef.current=null; setError(e instanceof Error?e.message:'加载存档失败'); setLoading(false); } }); return () => { alive = false; }; }, [port, saveId, allowMemoryFallback]);
  const commands = useMemo(() => { const names = ['saveRoomBlueprint','placeRoom','setRate','requestVisual','openHotel','advanceDay'] as const; return Object.fromEntries(names.map(name => [name, (...args:any[]) => { pendingCount.current+=1; setCommandPending(true); const run = queue.current.then(async()=>{ const current=stateRef.current; if(!current)return; try { const next=await (createGameCommands(activePort)[name] as any)(current,...args); stateRef.current=next; setState(next); setError(null); } catch(e){setError(e instanceof Error?e.message:'操作失败')} }); queue.current=run.finally(()=>{pendingCount.current-=1;if(pendingCount.current===0)setCommandPending(false)}); return run; }])) as Ctx['commands']; }, [activePort]);
