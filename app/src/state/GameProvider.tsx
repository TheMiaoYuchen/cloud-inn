...
type Ctx = { state: GameState | null; loading: boolean; commandPending: boolean; visualPending: boolean; error: string | null; commands: { [K in keyof Commands]: (...args: any[]) => Promise<void> }; visualProvider: VisualProvider };
...
export function GameProvider({ children, savePort: port, saveId = 'save-1', visualProvider, allowMemoryFallback = false }: { children: React.ReactNode; savePort: SavePort; saveId?: string; visualProvider?: VisualProvider; allowMemoryFallback?: boolean }) {
  ...
  const [commandPending, setCommandPending] = useState(false); const [visualPending, setVisualPending] = useState(false); const stateRef = useRef(state); stateRef.current = state; const queue = useRef(Promise.resolve()); const pendingCount = useRef(0);
  ...
  const commands = useMemo(() => { const names = ['saveRoomBlueprint','placeRoom','setRate','requestVisual','openHotel','advanceDay'] as const; return Object.fromEntries(names.map(name => [name, (...args:any[]) => { pendingCount.current+=1; setCommandPending(true); if (name === 'requestVisual') setVisualPending(true); const run = queue.current.then(async()=>{ const current=stateRef.current; if(!current)return; try { const next=await (createGameCommands(activePort)[name] as any)(current,...args); stateRef.current=next; setState(next); setError(null); } catch(e){setError(e instanceof Error?e.message:'操作失败')} }); queue.current=run.finally(()=>{pendingCount.current-=1;if(name==='requestVisual')setVisualPending(false);if(pendingCount.current===0)setCommandPending(false)}); return run; }])) as Ctx['commands']; }, [activePort]);
