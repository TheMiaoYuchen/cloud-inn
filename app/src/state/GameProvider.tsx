...
import { createSpaceCommands } from '../application/spaceCommands';
import { createFacilityCommands } from '../application/facilityCommands';
...
type Commands = ReturnType<typeof createGameCommands> & ReturnType<typeof createBuildingCommands> & ReturnType<typeof createSpaceCommands> & ReturnType<typeof createFacilityCommands>;
...
type ReloadOptions = { resetDraft?: boolean };
type Ctx = { state: GameState | null; building: BuildingProjection | null; loading: boolean; commandPending: boolean; visualPending: boolean; error: string | null; startupNotice: StartupNotice | null; reload(options?: ReloadOptions): Promise<boolean>; commands: { [K in keyof Commands]: (...args: any[]) => Promise<boolean> }; visualProvider: VisualProvider };
...
function draftFromState(state: GameState): RoomDraft {
  return state.roomBlueprint
    ? { name: state.roomBlueprint.name, cells: structuredClone(state.roomBlueprint.cells), activeZone: 'bedroom', tool: 'paint', visualPending: false }
    : { name: '', cells: [], activeZone: 'bedroom', tool: 'paint', visualPending: false };
}
...
export function GameProvider({ children, savePort: port, saveId = 'save-1', visualProvider, allowMemoryFallback = false, nowMs = Date.now, millisecondsPerGameDay = OPERATIONS_DAY_INTERVAL_MS[1] }: { children: React.ReactNode; savePort: SavePort; saveId?: string; visualProvider?: VisualProvider; allowMemoryFallback?: boolean; nowMs?: () => number; millisecondsPerGameDay?: number }) {
  ...
  const [commandPending, setCommandPending] = useState(false); const [visualPending, setVisualPending] = useState(false); const stateRef = useRef(state); stateRef.current = state; const queue = useRef<Promise<unknown>>(Promise.resolve()); const pendingCount = useRef(0); const outstanding = useRef(new Set<Promise<unknown>>()); const closingRef = useRef(false);
  ...
  useEffect(() => {
    ...
    initRef.current.task.then(result => {
      ...
      setDraft(draftFromState(result.state));
  ...
  const commands = useMemo(() => { const names = ['takeLoan','repayLoan','setDifficulty','setTimeSpeed','checkpointOfflineTime','initializeOperations','configureDepartment','renovateRoomOffer','setRoomPricePolicy','setAutomaticPricing','saveRoomSeries','syncRoomSeries','chooseCorridorTemplate','placeRoomVariant','saveRoomBlueprint','placeRoom','setRate','requestVisual','requestDesignVisuals','openHotel','advanceDay','advanceOperationsDays','settleOffline','initializeContentScale','purchaseFloor','copyFloor','syncFloorTemplate','savePublicSpaceBlueprint','placePublicSpace','configureFacility','developSignatureOffering','selectSignatureOffering','setFacilityEnabled'] as const; return Object.fromEntries(names.map(name => [name, (...args:any[]) => { if (closingRef.current) return Promise.resolve(false); const generation = lifecycleRef.current.generation; pendingCount.current+=1; setCommandPending(true); if (name === 'requestVisual' || name === 'requestDesignVisuals') setVisualPending(true); const run = queue.current.then(async()=>{ if (lifecycleRef.current.generation !== generation) return false; const current=stateRef.current; if(!current)return false; try { const commandArgs = name === 'advanceDay' && args.length === 0 ? [nowMs()] : args; const commandSet = { ...createGameCommands(activePort), ...createBuildingCommands(activePort), ...createSpaceCommands(activePort), ...createFacilityCommands(activePort) }; const next=await (commandSet[name] as any)(current,...commandArgs); if (lifecycleRef.current.generation !== generation) return false; stateRef.current=next; setState(next); setError(null); return true; } catch(e){if (lifecycleRef.current.generation === generation) setError(e instanceof Error ? e.message : typeof e === 'string' ? e : '操作失败'); return false} }); const settled=run.finally(()=>{if (lifecycleRef.current.generation !== generation) return; pendingCount.current-=1;if(name==='requestVisual' || name === 'requestDesignVisuals')setVisualPending(false);if(pendingCount.current===0)setCommandPending(false)}); queue.current=settled; outstanding.current.add(settled); void settled.then(()=>outstanding.current.delete(settled),()=>outstanding.current.delete(settled)); return run; }])) as Ctx['commands']; }, [activePort, nowMs]);
  const reload = useCallback((options: ReloadOptions = {}): Promise<boolean> => {
    if (closingRef.current || lifecycleRef.current.generation !== renderGeneration || lifecycleRef.current.saveId !== saveId) return Promise.resolve(false);
    const generation = renderGeneration;
    pendingCount.current += 1;
    setCommandPending(true);
    const run = queue.current.then(async () => {
      if (lifecycleRef.current.generation !== generation) return false;
      try {
        const next = await activePort.load(saveId as any);
        if (!next) throw new Error('存档不存在');
        if (lifecycleRef.current.generation !== generation) return false;
        stateRef.current = next;
        setState(next);
        if (options.resetDraft !== false) setDraft(draftFromState(next));
        setError(null);
        return true;
      } catch (failure) {
        if (lifecycleRef.current.generation === generation) setError(failure instanceof Error ? failure.message : '加载存档失败');
        return false;
      }
    });
    const settled = run.finally(() => {
      ...
      pendingCount.current -= 1;
      if (pendingCount.current === 0) setCommandPending(false);
    });
    queue.current = settled;
    outstanding.current.add(settled);
    void settled.then(() => outstanding.current.delete(settled), () => outstanding.current.delete(settled));
    return run;
  }, [activePort, renderGeneration, saveId]);
  ...
  useEffect(() => { const flush = (event: Event) => { closingRef.current = true; const detail = (event as CustomEvent<{ settle(task: Promise<unknown>): void }>).detail; detail?.settle(Promise.allSettled([...outstanding.current])); }; window.addEventListener('cloudinn:flush-requested', flush); return () => window.removeEventListener('cloudinn:flush-requested', flush); }, []);
