...
export function App() {
  ...
  return <main className="app-shell">
    ...
    <section className="intro">
      ...
      <p>{workspace === "floor" ? "这一层共 1200㎡。拖拽蓝图库中已设计的空间，按面积组织客房、接待与设施，并为后续经营模拟建立动线。" : project.designKind === "room" ? "从客房类型和床型开始，再添置家具。每件家具都能单独决定材质和风格；留空时由模型为整间房随机搭配。" : "选择一处酒店功能区域，再写下它的风格、光照、陈设和特殊元素。模型会将它组织为可用于后续建设的空间蓝图。"}</p>
    </section>
