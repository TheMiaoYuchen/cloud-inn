...
export function App() {
  ...
  function beginDesign(kind: "room" | "zone") {
    if (project.designKind === kind) return;
    ...
    try {
      const response = await fetch("/api/generate", {
        ...
        body: JSON.stringify(project.designKind === "zone"
          ? { designKind: "zone", zoneTypeId: project.zoneTypeId, stylePrompt: project.stylePrompt }
          : { designKind: "room", roomTypeId: project.roomTypeId, bedTypeId: project.bedTypeId, furniture: project.furniture, stylePrompt: project.stylePrompt }),
      });
  ...
  return <main className="app-shell">
    ...
    <section className="intro">
      <div className="design-switcher" role="tablist" aria-label="设计内容"><button type="button" role="tab" aria-selected={project.designKind === "room"} className={project.designKind === "room" ? "selected" : ""} onClick={() => beginDesign("room")}>设计客房</button><button type="button" role="tab" aria-selected={project.designKind === "zone"} className={project.designKind === "zone" ? "selected" : ""} onClick={() => beginDesign("zone")}>设计功能区域</button></div>
      <p className="eyebrow">{project.designKind === "room" ? "一间房，一种情绪" : "一处区域，一种体验"}</p>
      <h1>{project.designKind === "room" ? "把你想住进去的客房，变成一张图。" : "把酒店里的功能区域，变成一张图。"}</h1>
      <p>{project.designKind === "room" ? "从客房类型和床型开始，再添置家具。每件家具都能单独决定材质和风格；留空时由模型为整间房随机搭配。" : "选择一处酒店功能区域，再写下它的风格、光照、陈设和特殊元素。模型会将它组织为可用于后续建设的空间蓝图。"}</p>
    </section>
    <div className="studio">
      <section className="controls" aria-label="设计工具">
        ...
        <fieldset>
          <legend>{project.designKind === "room" ? "05 · 写下想象" : "03 · 写下想象"}</legend>
